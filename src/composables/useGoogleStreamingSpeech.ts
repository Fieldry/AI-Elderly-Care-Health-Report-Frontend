...
function formatTranscript(baseText: string, finalText: string, interimText: string): string {
...
function resampleToPcm16(input: Float32Array, sourceSampleRate: number): Int16Array {
  if (!input.length) {
  ...
  if (sourceSampleRate === TARGET_SAMPLE_RATE) {
    ...
    for (let i = 0; i < input.length; i += 1) {
      direct[i] = floatToInt16Sample(input[i])
    }
    return direct
  }
  ...
  for (let targetOffset = 0; targetOffset < targetLength; targetOffset += 1) {
    const nextSourceOffset = Math.min(
      input.length,
      Math.round((targetOffset + 1) * sampleRateRatio)
    )
...
export function useGoogleStreamingSpeech(options: UseGoogleStreamingSpeechOptions = {}) {
  ...
  function emitTranscript() {
    options.onTranscript?.(
      formatTranscript(baseText, finalTranscript, interimTranscript.value)
    )
  }
  ...
  async function setupAudioPipeline(stream: MediaStream) {
    if (!AudioContextConstructor) {
      throw new Error('当前浏览器不支持音频上下文，无法使用 Google 语音输入。')
    }
  ...
  async function start(sessionId: string, initialText = '') {
    if (!isSupported.value) {
      errorMessage.value = '当前浏览器暂不支持 Google 流式语音输入。'
      ...
      const readyPromise = new Promise<void>((resolve, reject) => {
        ...
        socket.onmessage = (event) => {
          ...
          if (payload.type === 'error' && !readyHandled) {
            readyHandled = true
            errorMessage.value = payload.message || 'Google 语音服务启动失败。'
